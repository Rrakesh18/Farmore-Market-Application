...
function CropCard({ crop, onBuyClick, onEditClick, onDeleteClick, page }) {
    const { user } = useAuth();
    const [showDetails, setShowDetails] = useState(false);
    const { t } = useLanguage();
    
    // Ee No-DTO backend lo, farmer details direct ga 'farmer' ane property lo vastayi
    const farmerInfo = crop.farmer || { id: null, name: 'Unknown', email: 'N/A', village: 'N/A', phone: 'N/A' };
        ...
        <div className="card h-100 shadow">
            ...
            <div className="card-body d-flex flex-column">
                ...
                <div className="d-flex justify-content-between align-items-center mt-auto">
                    ...
                    {/* Farmer view on All Listings page */}
                    {user?.role === 'farmer' && page === 'all-listings' && user.id === farmerInfo.id && (
                         <div>
                     ...
                     {/* Farmer view on Dashboard (No buttons) */}
                    {user?.role === 'farmer' && page === 'dashboard' && (
                        <p className="text-muted small fst-italic w-100 text-center">Your Listing</p>
                    )}
                </div>
                ...
                {showDetails && (
                    <div className="mt-3 p-3 bg-light rounded small">
                        ...
                        <p className="mb-1"><strong>{t('contactInfo')}:</strong> {farmerInfo.email} / {farmerInfo.phone}</p>
