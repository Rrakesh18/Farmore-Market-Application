...
public class CropController {
    ...
    @PostMapping
    public ResponseEntity<?> createCrop(@RequestBody Map<String, Object> payload) {
        try {
            return ResponseEntity.ok(cropService.createCrop(payload));
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.badRequest().body(java.util.Map.of("error", e.getMessage() != null ? e.getMessage() : "Unknown error", "type", e.getClass().getName()));
        }
    }
